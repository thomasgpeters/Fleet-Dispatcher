...
CREATE TABLE trailer_type (
    ...
    code  TEXT NOT NULL UNIQUE,
    name  TEXT NOT NULL,
    -- Rigs are colour-coded by trailer type on the board/fleet views (a rig's
    -- colour = its trailer type's colour). Hex incl. '#'; NULL = neutral default.
    color_hex TEXT
);
...
-- Curated avatar palette: users self-pick a colour in their profile; drivers
-- (incl. login-less ones) can be assigned one by an admin. The person glyph on
-- the board/fleet views is tinted with this colour (shared across all clients).
CREATE TABLE avatar_color (
    id    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code  TEXT NOT NULL UNIQUE,
    name  TEXT NOT NULL,
    hex   TEXT NOT NULL           -- includes leading '#', e.g. '#2f9e94'
);
...
CREATE TABLE app_user (
    ...
    avatar_color_id   INTEGER REFERENCES avatar_color(id),  -- self-picked person-icon colour
...
CREATE TABLE driver (
    ...
    -- Admin-assignable person-icon colour for this driver record. Render
    -- precedence: this → the linked user's avatar_color → a deterministic
    -- colour derived from the name (so every driver always has a colour).
    avatar_color_id INTEGER REFERENCES avatar_color(id),
    created_at     TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ===========================================================================
-- Vehicles & rig bundles  (Feature 7 — per-asset model; see docs/TODO.md and
-- docs/INTEGRATION_SMITTY.md). Each physical asset (tractor OR trailer) is its
-- own VIN-keyed `vehicle`; a `rig_bundle` is the TEMPORAL combination of a power
-- unit + trailer + driver(s) (teams supported), giving full "who/what combined
-- when" history. `vehicle` is the canonical entity shared with Smitty Services
-- (by VIN). Added ALONGSIDE `equipment` (Phase 1); load/trip/position/seed
-- repoint in a later phase.
-- ===========================================================================
CREATE TABLE asset_type (
    id    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code  TEXT NOT NULL UNIQUE,   -- tractor | trailer
    name  TEXT NOT NULL
);

CREATE TABLE vehicle_status (
    id    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code  TEXT NOT NULL UNIQUE,   -- in_service | out_of_service | in_maintenance | retired
    name  TEXT NOT NULL
);

CREATE TABLE bundle_driver_role (
    id    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code  TEXT NOT NULL UNIQUE,   -- primary | co_driver
    name  TEXT NOT NULL
);

-- Who owns a vehicle, and who covers its maintenance/repair cost. These can
-- DIVERGE (a leased trailer is owned by a lessor but Fleet-maintained; an
-- owner-operator's tractor is O/O-owned and O/O-maintained). See vehicle_lease.
CREATE TABLE owner_type (
    id    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code  TEXT NOT NULL UNIQUE,   -- fleet | owner_operator | lessor
    name  TEXT NOT NULL
);

CREATE TABLE maint_responsibility (
    id    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code  TEXT NOT NULL UNIQUE,   -- fleet | owner_operator | lessor
    name  TEXT NOT NULL
);
...
CREATE TABLE vehicle (
    id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    vin               TEXT UNIQUE,                       -- 17-char when present
    unit_number       TEXT NOT NULL UNIQUE,              -- human-facing fleet #
    ...
    plate             TEXT,
    dot_number        TEXT,
    fuel_type         TEXT,                              -- diesel | gas | electric | …
    odometer_miles    INTEGER CHECK (odometer_miles IS NULL OR odometer_miles >= 0),
    odometer_as_of    TIMESTAMPTZ,
    vehicle_status_id INTEGER NOT NULL REFERENCES vehicle_status(id),
    in_service        BOOLEAN NOT NULL DEFAULT TRUE,
    -- Ownership + who bears repair cost (may diverge — see vehicle_lease).
    owner_type_id           INTEGER NOT NULL REFERENCES owner_type(id),
    owner_driver_id         UUID REFERENCES driver(id),   -- the owner-operator, when owner_type=owner_operator
    owner_name              TEXT,                         -- external owner/lessor name, when applicable
    maint_responsibility_id INTEGER NOT NULL REFERENCES maint_responsibility(id),  -- who covers repairs
    smitty_vehicle_id TEXT,                              -- Smitty correlation (their SMALLINT id, as text)
    created_at        TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Type-specific specs (1:1 with a vehicle, chosen by asset_type). Kept typed —
-- tractor and trailer specs are different domains — and serialized to JSON only
-- when publishing to Smitty.
CREATE TABLE tractor_spec (
    vehicle_id    UUID PRIMARY KEY REFERENCES vehicle(id) ON DELETE CASCADE,
    power_unit_id INTEGER REFERENCES power_unit(id),
    horsepower    SMALLINT CHECK (horsepower IS NULL OR horsepower > 0),
    num_axles     SMALLINT  CHECK (num_axles IS NULL OR num_axles > 0),
    has_sleeper   BOOLEAN NOT NULL DEFAULT FALSE
);

CREATE TABLE trailer_spec (
    vehicle_id          UUID PRIMARY KEY REFERENCES vehicle(id) ON DELETE CASCADE,
    trailer_type_id     INTEGER REFERENCES trailer_type(id),
    deck_length_ft      SMALLINT CHECK (deck_length_ft IS NULL OR deck_length_ft > 0),
    weight_capacity_lbs INTEGER  CHECK (weight_capacity_lbs IS NULL OR weight_capacity_lbs > 0),
    has_ramps           BOOLEAN NOT NULL DEFAULT FALSE,
    has_duals           BOOLEAN NOT NULL DEFAULT FALSE,
...
-- A temporal rig combination: a power unit + (optional) trailer working
-- together over [effective_from, effective_to). effective_to NULL = current.
CREATE TABLE rig_bundle (
    id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    power_vehicle_id   UUID NOT NULL REFERENCES vehicle(id),   -- an asset_type=tractor
    trailer_vehicle_id UUID REFERENCES vehicle(id),            -- an asset_type=trailer (NULL = bobtail)
    effective_from     TIMESTAMPTZ NOT NULL DEFAULT now(),
    effective_to       TIMESTAMPTZ,
    created_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT rig_bundle_period CHECK (effective_to IS NULL OR effective_to >= effective_from),
    CONSTRAINT rig_bundle_distinct CHECK (trailer_vehicle_id IS NULL OR trailer_vehicle_id <> power_vehicle_id)
);

CREATE INDEX idx_rig_bundle_power ON rig_bundle (power_vehicle_id, effective_to);
CREATE INDEX idx_rig_bundle_trailer ON rig_bundle (trailer_vehicle_id, effective_to);

-- Driver(s) on a bundle — supports teams (2+ drivers), each with a role.
CREATE TABLE rig_bundle_driver (
    id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    rig_bundle_id  UUID NOT NULL REFERENCES rig_bundle(id) ON DELETE CASCADE,
    driver_id      UUID NOT NULL REFERENCES driver(id),
    driver_role_id INTEGER NOT NULL REFERENCES bundle_driver_role(id),
...
-- A lease on a vehicle. Captures the cases where maintenance responsibility
-- diverges from ownership: a lessor owns the asset, a lessee (Fleet OR an
-- owner-operator) holds the lease, and the lease names who covers maintenance for
-- its term. E.g. Fleet leases a trailer it must maintain; or an owner-operator
-- leases their own trailer and responsibility follows that agreement.
CREATE TABLE vehicle_lease (
    id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    vehicle_id              UUID NOT NULL REFERENCES vehicle(id) ON DELETE CASCADE,
    lessor_name             TEXT,                             -- who leases it out (owner)
    lessee_type_id          INTEGER NOT NULL REFERENCES owner_type(id),  -- fleet | owner_operator (who holds the lease)
    lessee_driver_id        UUID REFERENCES driver(id),       -- the owner-operator lessee, when applicable
...
CREATE TABLE channel_member_role (
    id    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code  TEXT NOT NULL UNIQUE,    -- owner | admin | member
    name  TEXT NOT NULL
);

-- A member's standing in a channel. 'muted' / 'banned' may carry an expiry
-- (channel_member.restricted_until); 'active' is the default.
CREATE TABLE channel_member_status (
    id    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code  TEXT NOT NULL UNIQUE,    -- active | muted | banned
    name  TEXT NOT NULL
);
...
-- Membership of a user in a channel, with their role, standing, and read
-- position. member_role_id drives who may post in broadcast channels (owner/
-- admin only); member_status_id + restricted_until govern mute/ban (enforced by
-- a LogicBank rule in als-extensions/).
...
-- A forum topic (thread) within a channel — splits a group/board into focused
-- conversations (e.g. a topic per load/lane/region). Messages with topic_id
-- NULL belong to the channel's default ("General") stream.
CREATE TABLE channel_topic (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    channel_id  UUID NOT NULL REFERENCES channel(id) ON DELETE CASCADE,
    name        TEXT NOT NULL,
    created_by  UUID NOT NULL REFERENCES app_user(id),
    is_closed   BOOLEAN NOT NULL DEFAULT FALSE,  -- closed topics are read-only
    created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX idx_channel_topic_channel ON channel_topic (channel_id);

-- A text message posted to a channel. reply_to_id supports threaded replies;
-- topic_id places the message in a forum topic (NULL = the General stream).
CREATE TABLE message (
    ...
    -- Kept as a plain UUID (no self-FK to message(id)): clients resolve the quoted
    -- message by id via the API, and dropping the self-relationship keeps ALS's
    -- generated model simpler. (This was NOT the cause of the historic new-message
    -- 500 — that was safrs doing an existence check on an empty UUID id when a
    -- client omitted `data.id`; fixed by clients sending a generated id. See
    -- DEVELOPMENT_LOG 2026-07-30.)
    reply_to_id UUID,
...
CREATE INDEX idx_message_topic_posted ON message (topic_id, posted_at);

...
| POST 500 `InFailedSqlTransaction` + earlier `WHERE …id = ''::UUID` | A nullable FK arrived as `''` not `NULL` (e.g. `message.reply_to_id` on a non-reply post); LogicBank's parent-load casts `''::uuid` and aborts the txn. Fixed by `als-extensions/logic_discovery/empty_fk_coercion.py` (`''`→`NULL` on set) — reinstall with `make als-extensions` + restart. |

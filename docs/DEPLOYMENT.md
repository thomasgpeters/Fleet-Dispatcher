...
| POST 500 `InFailedSqlTransaction` + earlier `WHERE …id = ''::UUID` on a **create** | The client omitted `data.id`, so safrs ran an existence check `get('')` on the UUID PK → `''::uuid` aborts the txn. **Fix = clients send a generated UUID `data.id`** (mobile `createResource`, desktop `postJson`, smoke-check all do now). Purely client-side — no ALS change. Verify: `POST` with `"id":"<uuid>"` in the body returns 201. |
